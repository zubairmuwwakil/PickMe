...
// V6 adds one column: `StoredMerchant.placeID`, Apple's persistent place identifier.
//
// Nullable, and every existing row arrives here with it nil — which is its correct value. There
// is no offline function from the synthetic `"Metro@43.65,-79.38"` identifier those rows carry to
// an `MKMapItem.Identifier`; recovering one needs a live MapKit round trip per merchant, for a
// place that may no longer exist, with no guarantee the pin that comes back is the one the owner
// confirmed. A stage that guessed would bind a confirmed category to the shop next door, which is
// strictly worse than the orphan it set out to fix.
//
// So this column is backfilled lazily, by `MerchantIdentity`, the next time the owner is actually
// standing somewhere MapKit hands us the id. Until then the match ladder resolves those rows on
// their legacy identifier and their coordinates, exactly as before. Nothing is rewritten and
// nothing is invented at migration time.
//
// Every other entity is byte-identical to V5; `SchemaVersionTests.testV6ChangesOnlyStoredMerchant`
// is what keeps anything else from riding along in this migration.
extension CardCopilotSchemaV6 {
    @Model
    ...
    public final class StoredMerchant {
        ...
        /// Apple's persistent place identifier (`MKMapItem.Identifier.rawValue`), when MapKit
        /// supplied one. Distinct from `identifier` on purpose: `identifier` is frozen at whatever
        /// string created this row and is still what predictions and purchases join on, while this
        /// is *evidence about* the row's identity that later encounters can strengthen. Written
        /// only by `MerchantIdentity.backfill`, never re-keyed.
        ...
        public init(name: String, identifier: String? = nil, placeID: String? = nil,
