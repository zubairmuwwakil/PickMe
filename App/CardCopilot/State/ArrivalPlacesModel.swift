...
struct MonitoredArrivalPlace: Identifiable {
    ...
    func contains(_ merchant: NearbyMerchant) -> Bool {
...
final class ArrivalPlacesModel {
    ...
    func refresh() async {
        ...
        do {
            ...
            savedMerchants = try context.fetch(FetchDescriptor<StoredMerchant>()).map {
                NearbyMerchant(id: $0.identifier ?? $0.id.uuidString, name: $0.name,
    ...
    func search(_ query: String) async {
        ...
        do {
            ...
            searchResults = rankNearbyMerchants(results.filter(\.hasMonitorableLocation))
            isSearching = false
            hasSearched = true
        } catch {
            guard !Task.isCancelled else { return }
    ...
    func preference(for merchant: NearbyMerchant) -> ArrivalAlertPreference? {
    ...
    func isMuted(_ merchant: NearbyMerchant) -> Bool { ambient.isMerchantMuted(merchant.id) }

    func isMonitoring(_ merchant: NearbyMerchant) -> Bool {
        runtimeStatus.locationAlways && monitoredPlaces.contains { $0.contains(merchant) }
    }

    func latestArrivalExplanation(for merchant: NearbyMerchant) -> ArrivalExplanationSnapshot.Match? {
    ...
    func merchant(for preference: ArrivalAlertPreference) -> NearbyMerchant {
        ...
        return NearbyMerchant(id: saved?.id ?? preference.locationIdentifier ?? preference.merchantKey,
    ...
    var otherSavedMerchants: [NearbyMerchant] {
    ...
    @discardableResult
    func save(_ scope: ArrivalAlertScope, merchant: NearbyMerchant,
