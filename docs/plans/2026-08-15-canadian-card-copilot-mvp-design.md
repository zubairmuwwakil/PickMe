...
**Valuation-sensitivity layer.** Every card is scored three ways — at the owner's declared value, at the guaranteed cash floor (`floorNetValueCad`), and at the published benchmark (`aspirationalNetValueCad`). Ranking always uses the declared value; the other two only detect dependency:

- **Downside** (declared > floor): the winner is a points card that wins *because of* an optimistic assumption → *"Assumes your points are worth 1.80¢ each. Below about 1.67¢, MBNA wins instead."*
- **Upside** (declared at floor): a points card would overtake the conservative winner if points were worth more → *"Assumes your points are worth 1.00¢ each. Above about 1.25¢, Cobalt wins instead."* Disclosed only when the flip lands within the published benchmark; past that it is noise (pharmacy needs 2.83¢ > 2.2¢, so it stays silent).

Without the upside half, ranking at the floor would silently suppress every points recommendation and the owner would never learn what the conservatism costs. The breakeven is analytic and cross-validated against bisection over the real engine (`BreakevenCrossValidationTests`) — a disclosed number that disagrees with actual behaviour is worse than no disclosure.
